...
export default function DoctorProfileScreen() {
    ...
    useEffect(() => {
        async function getPhone() {
            try {
                if (phone) {
                    const specificProfile: any = await db.getFirstAsync('SELECT id, phone, care_mode FROM patient_profiles WHERE phone = ?', [phone]);
                    if (specificProfile) {
                        setPatientPhone(specificProfile.phone);
                        setPatientId(specificProfile.id);
                        return; // Exit early if we found the exact profile
                    }
                }

                // Fallback (only if phone wasn't provided or not found)
