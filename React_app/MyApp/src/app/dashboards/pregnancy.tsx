...
function VerificationBanner({ status }: { status: string }) {
    const cfg: Record<string, { icon: string; title: string; subtitle: string; color: string; bg: string; border: string }> = {
        pending: {
            icon: '⏳', color: '#F59E0B', bg: '#2D2010', border: '#F59E0B40',
            title: 'Document Under Review',
            subtitle: 'Your CHW worker will verify your document shortly.',
        },
        approved: {
            icon: '✅', color: '#10B981', bg: '#0D2A1E', border: '#10B98140',
            title: 'Document Approved',
            subtitle: 'Your pregnancy has been verified. Welcome to your journey!',
        },
        rejected: {
            icon: '❌', color: '#EF4444', bg: '#2A0D0D', border: '#EF444440',
            title: 'Document Rejected',
            subtitle: 'Please re-upload a clearer photo of your ANC card or ultrasound report.',
        },
    };
    const c = cfg[status];
    if (!c) return null;
    return (
        <View style={[banner.box, { backgroundColor: c.bg, borderColor: c.border }]}>
            <Text style={banner.icon}>{c.icon}</Text>
            <View style={{ flex: 1 }}>
                <Text style={[banner.title, { color: c.color }]}>{c.title}</Text>
                <Text style={banner.sub}>{c.subtitle}</Text>
            </View>
        </View>
    );
}
const banner = StyleSheet.create({
    ...
    sub: { color: '#94A3B8', fontSize: 12, lineHeight: 16 },
});
            ...
            async function loadData() {
                ...
                try {
                    ...
                    const user: any = await db.getFirstAsync(
                        'SELECT full_name, verification_status FROM patient_profiles WHERE phone = ?',
                        [phone]
                    );
                    if (user && user.full_name) setPatientName(user.full_name.split(' ')[0]);
                    if (user && user.verification_status) setVerificationStatus(user.verification_status);
    ...
    return (
        <View style={styles.container}>
            ...
            <SafeAreaView style={styles.safeArea} edges={['top']}>
                ...
                >
                    {/* ── Verification Status Banner ── */}
                    {(verificationStatus === 'pending' || verificationStatus === 'rejected' || verificationStatus === 'approved') && (
